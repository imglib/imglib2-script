...
public class Benchmark
{
	...
	static public Img< FloatType > scriptCorrectIllumination( final Img< ? extends RealType< ? > > img, final Img< ? extends RealType< ? > > brightfield, final Img< ? extends RealType< ? > > darkfield, final double mean ) throws Exception
	{
		p( "Start script (correct illumination)..." );
		long t0 = System.nanoTime();
		Img< FloatType > corrected = Compute.inFloats( 2, new Multiply( new Divide( new Subtract( img, brightfield ), new Subtract( brightfield, darkfield ) ), mean ) );
		p( "  elapsed: " + ( System.nanoTime() - t0 ) / 1000000.0 + " image " );
		return corrected;
	}

	static public Img< FloatType > correctIllumination( final Img< ? extends RealType< ? > > img, final Img< ? extends RealType< ? > > brightfield, final Img< ? extends RealType< ? > > darkfield, final double mean )
	{
		p( "Start direct (correct illumination)..." );
		long t0 = System.nanoTime();
		Img< FloatType > corrected = new ArrayImgFactory<>( new FloatType() ).create( img );
		final Cursor< FloatType > c = corrected.cursor();
		final Cursor< ? extends RealType< ? > > ci = img.cursor(),
				cb = brightfield.cursor(), cd = darkfield.cursor();
		while ( c.hasNext() )
		{
			c.fwd();
			ci.fwd();
			cb.fwd();
			cd.fwd();
			c.get().setReal( ( ( ci.get().getRealDouble() - cb.get().getRealDouble() ) / ( cb.get().getRealDouble() - cd.get().getRealDouble() ) ) * mean );
		}
		p( "  elapsed: " + ( System.nanoTime() - t0 ) / 1000000.0 + "  image" );
		return corrected;
	}

	static public < R extends RealType< R > > Img< FloatType > scriptHeavyOperations( final Img< R > img ) throws Exception
	{
		p( "Start script with heavy operations..." );
		long t0 = System.currentTimeMillis();
		Img< FloatType > corrected = Compute.inFloats( 1, new Multiply( new ASin( new Sin( new Divide( new Pow( new Sqrt( img ), 2 ), new Pow( new Cbrt( img ), 3 ) ) ) ), img ) );
		p( "  elapsed: " + ( System.currentTimeMillis() - t0 ) );
		return corrected;
	}

	static public Img< FloatType > heavyOperations( final Img< ? extends RealType< ? > > img )
	{
		...
		while ( c.hasNext() )
		{
			c.fwd();
			ci.fwd();
			c.get().setReal( Math.asin( Math.sin( Math.pow( Math.sqrt( ci.get().getRealDouble() ), 2 ) / Math.pow( Math.cbrt( ci.get().getRealDouble() ), 3 ) ) ) * ci.get().getRealDouble() );
		}
		p( "  elapsed: " + ( System.currentTimeMillis() - t0 ) );
		return corrected;
	}

	static public Img< FloatType > sum( final Img< ? extends RealType< ? > > img ) throws Exception
	{
		...
		r.setPosition( pos );
		System.out.println( "After varargs addition, pixel at 348,95: " + r.get().getRealDouble() + " which is 4 * val: " + ( c.get().getRealDouble() * 4 == r.get().getRealDouble() ) );

		return result;
	}

	static public Img< FloatType > differenceFn( final Img< ? extends RealType< ? > > img ) throws Exception
	{
	...
	static public Img< FloatType > differenceCompFn( final Img< ? extends RealType< ? > > img ) throws Exception
	{
	...
	/**
	 * Copy the contents from img to an ImagePlus; assumes containers are
	 * compatible.
	 */
	static public ImagePlus copyToFloatImagePlus( final Img< ? extends RealType< ? > > img, final String title )
	{
		...
		ImagePlusImg< FloatType, ? > iml = factory.create( img );
	...
	public static void main( String[] args )
	{
		try
		{
			...
			Img< UnsignedByteType > img1 = ( Img< UnsignedByteType > ) new ImgOpener().openImgs( src ).get( 0 );
			...
			Img< UnsignedByteType > img = ArrayImgs.unsignedBytes( 512, 512 );
			...
			/*
			 * GaussianConvolutionReal<UnsignedByteType> gauss = new
			 * GaussianConvolutionReal<UnsignedByteType>( img, new
			 * OutOfBoundsMirrorFactory(), 60 ); gauss.process();
			 * 
			 * System.out.println( "Gauss processing time: " +
			 * gauss.getProcessingTime() );
			 * 
			 * Image<UnsignedByteType> brightfield = gauss.getResult();
			 */

			/*
			 * DownSample<UnsignedByteType> downSample = new
			 * DownSample<UnsignedByteType>( img, 0.25f ); downSample.process();
			 * Image<UnsignedByteType> down = downSample.getResult();
			 * 
			 * ImageJFunctions.show( down );
			 * 
			 * AffineModel2D model = new AffineModel2D(); model.set( 4.03f, 0,
			 * 0, 4.03f, 0, 0 );
			 * 
			 * ImageTransform<UnsignedByteType> imgTransform = new
			 * ImageTransform<UnsignedByteType>( brightfield, model, new
			 * LinearInterpolatorFactory<UnsignedByteType>( new
			 * OutOfBoundsStrategyMirrorFactory<UnsignedByteType>()) );
			...
			/*
			 * ImageJFunctions.show( img ); ImageJFunctions.show( brightfield );
			 */

			// A simulated brightfield image
			// Until Gauss is restored, just use an image with half-tone
			Img< UnsignedByteType > brightfield = img.factory().create( img );
			...
			// A black image: empty
			Img< UnsignedByteType > darkfield = img.factory().create( img );
